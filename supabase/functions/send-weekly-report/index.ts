...
serve(async (req) => {
  ...
  try {
    ...
    const weeklyEnabled = settings.weekly?.enabled || false;
    
    // Check if today is the configured day for weekly reports
    const today = new Date();
    const dayOfWeek = today.toLocaleDateString("en-US", { weekday: "lowercase" });
    const isConfiguredDay = settings.weekly?.day === dayOfWeek;
    
    if (!weeklyEnabled || !isConfiguredDay) {
      return new Response(JSON.stringify({ 
        message: !weeklyEnabled 
          ? "Weekly reports are disabled" 
          : `Today is not the configured day (${settings.weekly?.day})`,
        success: false
      }), {

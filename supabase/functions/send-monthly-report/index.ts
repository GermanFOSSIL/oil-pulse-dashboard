...
serve(async (req) => {
  ...
  try {
    ...
    const monthlyEnabled = settings.monthly?.enabled || false;
    
    // Check if today is the configured day for monthly reports
    const today = new Date();
    const dayOfMonth = today.getDate().toString();
    const isConfiguredDay = settings.monthly?.day === dayOfMonth;
    
    if (!monthlyEnabled || !isConfiguredDay) {
      return new Response(JSON.stringify({ 
        message: !monthlyEnabled 
          ? "Monthly reports are disabled" 
          : `Today is not the configured day (${settings.monthly?.day})`,
        success: false
      }), {
